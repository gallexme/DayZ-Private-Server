...
_listTalk = (position _refObj) nearEntities ["zZombie_Base",150];
...
canSee = {
	...
	if ((count _tPos > 0) and (count _zPos > 0)) then {
		_iSeeYou = terrainIntersectASL [_zPos, _tPos];
		if (!_iSeeYou) then {
			_iSeeYou = lineIntersects [_zPos, _tPos];
			};
		};
	_iSeeYou
};

{
	if (alive _x) then {
		...
		_chance = 1;
		_clearView = [position _x, position player] call canSee; 
		if ((_x distance player < dayz_areaAffect) and !(animationState _x == "ZombieFeed") and _clearView) then {
			[_x,"attack",(_chance),true] call dayz_zombieSpeak;
			//perform an attack
			_last = _x getVariable["lastAttack",0];
			_entHeight = (getPosATL _x) select 2;
			_delta = _pHeight - _entHeight;
			if ( ((time - _last) > 2) and ((_delta < 1.5) and (_delta > -1.5)) ) then {
